...
public class ChatController {
    ...
    private static final int STREAM_QUEUE_CAPACITY = 100;
    ...
    public SseEmitter chatStream(@RequestBody ChatRequest request) {
        ...
        try {
            ...
            logger.warn("SSE chat task rejected", e);
    ...
    public SseEmitter aiOps() {
        ...
        try {
            ...
            logger.warn("AI Ops task rejected", e);
            sendBusyError(emitter);
        }
    ...
    private void cleanupSessions() {
        ...
        sessions.entrySet().removeIf(entry -> now - entry.getValue().getLastAccessTime() > SESSION_TTL_MS);
        ...
        sessions.entrySet().stream()
                .sorted(Comparator.comparingLong(entry -> entry.getValue().getLastAccessTime()))
                .limit(sessions.size() - MAX_SESSION_COUNT)
    ...
    private void sendBusyError(SseEmitter emitter) {
        try {
            emitter.send(SseEmitter.event()
                    .name("message")
                    .data(SseMessage.error("server is busy, please retry later"), MediaType.APPLICATION_JSON));
            emitter.complete();
        } catch (IOException ex) {
